"""Agent with database persistence and proper authentication."""
...
from app.utils.session_db import (
    init_session_schema,
    create_session,
    get_session,
    add_message,
...
class RetailAgentDB:
    """Retail assistant agent with database persistence."""
    ...
    def __init__(self):
        ...
        init_session_schema()
        logger.info("RetailAgentDB initialized with %d tools", len(self.tools))
    ...
    def run(self, request: AgentRequest) -> AgentResponse:
        """
        Process a user query with database persistence.
        ...
        try:
            ...
            if not existing_session:
                ...
                logger.info("Created new session in DB: %s for user: %s", session_id, request.user_id)
            
            # Get conversation history from database
            messages = []
            if request.include_history:
                db_messages = get_session_messages(session_id, limit=10)
                for msg in db_messages[-6:]:  # Last 6 messages for context
                    if msg["role"] == "user":
...
_agent_instance: Optional[RetailAgentDB] = None


def get_agent_db() -> RetailAgentDB:
    """Get or create the global agent instance with DB persistence."""
    global _agent_instance
    if _agent_instance is None:
        _agent_instance = RetailAgentDB()
